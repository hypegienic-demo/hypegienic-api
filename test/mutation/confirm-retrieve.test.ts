...
describe('confirm-retrieve', () => {
  ...
  it('should return error if not requested retrieve', async() => {
  ...
  it('should return error for unregistered user', async() => {
    const {body:retrieveBody} = await requires(app.server)
      .post('/root')
      .set('Content-Type', 'multipart/graphql')
      .set('Authorization', 'chingyawhao')
      .field('graphql', `
      ...
      `)
      .expect(200)
    const retrieveLocker = retrieveBody?.data.requestRetrieveBack
    openedLocker = retrieveLocker
      ? {
          lockerName: retrieveLocker.locker.name,
          lockerUnitId: retrieveLocker.number - 1
        }
      : undefined
    const {body} = await requires(app.server)
      .post('/root')
  ...
  it('should return error if locker unit not closed', async() => {
    const {body} = await requires(app.server)
      .post('/root')
      .set('Content-Type', 'multipart/graphql')
      .set('Authorization', 'chingyawhao')
  ...
  it('should return order if success', async() => {
    if(openedLocker) {
      await app.utilities.physicalLockerStore.setLockerUnit(
        openedLocker.lockerName,
        openedLocker.lockerUnitId,
        'locked'
      )
    }
    const {body} = await requires(app.server)
    ...
    expect(body?.data.confirmRetrieve).toEqual({
      id: order?.id
    })
  })
})