...
describe('add-before-images', () => {
  ...
  it('should return error if request not found', async() => {
    ...
    addBeforeImages
      ...
      `)
    const {body} = await addBeforeImages.expect(200)
  ...
  it('should return error if not yet requested retrieve', async() => {
  ...
  it('should return order if success', async() => {
    ...
    expect(body?.data.addBeforeImages).toEqual({
      id: order?.id
    })
  })
})