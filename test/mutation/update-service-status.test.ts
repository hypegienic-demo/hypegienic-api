...
describe('update-service-status', () => {
  ...
  it('should return error if not uploaded before images', async() => {
      ...
      .field('graphql', `
        mutation {
          requestLocker(
            ...
            name: "Nike Huarache",
            serviceIds: ["${cleanService?.id}", "${waterRepellentService?.id}"]
          ) {
            id
            number
            locker {
  ...
  it('should return error if not include a valid service', async() => {
    ...
    const {body} = await requires(app.server)
      ...
      .field('graphql', `
        mutation {
          updateServiceStatus(
            orderId: "${order?.id}"
            servicesDone: ["${soleProtectService?.id}"]
          ) {
            id
          }
        }
    ...
    expect(body?.errors).toContain(
      'Please specify at least one pending service'
    )
  })
  ...
  it('should return order if success', async() => {
    ...
    expect(body1?.data.updateServiceStatus).toEqual({
      ...order,
    ...
    expect(body2?.data.updateServiceStatus).toEqual({
      ...order,
      status: 'delivered-store',
      services: expect.arrayContaining(
