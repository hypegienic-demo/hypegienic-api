...
describe('add-after-images', () => {
  ...
  it('should return error if request not found', async() => {
    ...
    addAfterImages
      ...
      `)
    const {body} = await addAfterImages.expect(200)
  ...
  it('should return error if not yet delivered to store', async() => {
  ...
  it('should return error if not all services are done', async() => {
    ...
    expect(body?.errors).toContain(
      "Order haven't done cleaning yet"
    )
  })

  it('should return error if no images attached', async() => {
    await requires(app.server)
      .post('/root')
      .set('Content-Type', 'multipart/graphql')
      .set('Authorization', 'chingyawhao')
  ...
  it('should return order if success', async() => {
    ...
    expect(body?.data.addAfterImages).toEqual({
      id: order?.id
    })
  })
})